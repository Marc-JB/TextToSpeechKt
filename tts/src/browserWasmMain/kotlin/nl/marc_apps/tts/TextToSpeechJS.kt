...
import org.w3c.speech.getSynthesis
...
internal class TextToSpeechJS(context: Window = window) : TextToSpeechInstance {
    ...
    override val language: String
        get() {
            ...
            return reportedLanguage.ifBlank {
                val defaultLanguage = getVoiceList().find { it.default }?.lang
    ...
    private val defaultVoice by lazy {
        getVoiceList().find { it.default }?.let { BrowserVoice(it) }
    }
    ...
    override val voices: Sequence<Voice> by lazy {
        getVoiceList().asSequence().map { BrowserVoice(it) }
    }
